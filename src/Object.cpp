...
void Label::Update(const orxCLOCK_INFO &_rstInfo)
{
    ...
    orxU32 u32Offset = vMousePosition.fX * 4 + vMousePosition.fY * 4 * static_cast<orxU32>(fWidth);
    orxASSERT(u32Offset < u32Bytes);

    orxCHAR acLabel[1024];
    orxString_NPrint(acLabel, sizeof(acLabel), "MOUSE: (%g, %g) LABEL: (%d, %d, %d)", vMousePosition.fX, vMousePosition.fY, au8Buf[u32Offset], au8Buf[u32Offset + 1], au8Buf[u32Offset + 2]);
    SetText(acLabel);
